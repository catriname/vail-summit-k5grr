...
**Custom Font Awesome menu font:** Regenerating is documented in **`docs/EXTRA_FONT_AWESOME_ICONS.md`**. Paste converter output into **`src/fonts/extra_font_awesome_icons_generated.h`** only (that file must exist next to **`extra_font_awesome_icons.c`**); keep **`extra_font_awesome_icons_shell.h`** and the small **`extra_font_awesome_icons.c`** wrapper unchanged.
